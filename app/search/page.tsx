...
import { Search, MapPin, ChevronRight, Loader2, X, Star, Wrench, AlertTriangle, SlidersHorizontal, Plus, ThermometerSun, Droplets } from 'lucide-react';
...
function SearchResults() {
  ...
  // Loodgieter diensten types
  const serviceTypes = [
    { value: 'all', label: 'Alle Diensten', icon: Wrench },
    { value: 'spoed-lekkage', label: 'Spoed Lekkage', icon: AlertTriangle },
    { value: 'sanitair', label: 'Sanitair', icon: Droplets },
    { value: 'cv-installatie', label: 'CV Installatie', icon: ThermometerSun },
    { value: 'riool-afvoer', label: 'Riool & Afvoer', icon: Wrench },
    { value: 'badkamer', label: 'Badkamer', icon: Droplets },
  ];
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 text-white py-12">
        <div className="container mx-auto px-4">
          <h1 className="font-serif text-3xl sm:text-4xl font-semibold mb-4 text-center">
            {searchQuery ? `Zoekresultaten voor "${searchQuery}"` : 'Vind een Loodgieter'}
          </h1>
          <p className="text-blue-100 text-center max-w-2xl mx-auto">
            Zoek in onze database van loodgieters in heel Nederland
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-7xl mx-auto">
          {/* Search & Filter Bar */}
          <Card className="p-4 mb-8 shadow-soft border-blue-100">
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              <div className="flex-1 relative">
                ...
                <Input
                  ...
                  className="pl-12 h-12 text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              ...
              <div className="hidden lg:flex items-center gap-3">
                ...
                <Button onClick={handleSearch} size="lg" className="h-12 px-6 bg-blue-600 hover:bg-blue-700 text-white">
          ...
          {hasActiveFilters && (
            <div className="flex flex-wrap items-center gap-2 mb-6">
              ...
              {searchQuery && (
                <span className="inline-flex items-center gap-1 px-3 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-full text-sm">
          ...
          <div className="mb-6">
            {loading ? (
              ...
              <p className="text-muted-foreground">
                <span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 bg-blue-600 text-white text-sm font-semibold rounded-full mr-2">{results.length}</span>
                loodgieters gevonden
              </p>
            )}
          </div>
          ...
          {loading ? (
            <div className="flex justify-center py-20">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Loodgieters zoeken...</p>
              </div>
            </div>
          ) : results.length === 0 ? (
            <Card className="p-8 sm:p-12 text-center border-blue-100">
              <div className="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-6">
                <Search className="w-8 h-8 text-blue-600" />
              </div>
              <h3 className="font-serif text-xl font-semibold mb-2">Geen Resultaten Gevonden</h3>
              <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                We konden geen loodgieters vinden die aan uw zoekopdracht voldoen.
                ...
                {results.slice(0, visibleResults).map((facility, index) => (
                  <Fragment key={facility.slug}>
                    <Link
                      ...
                      className="group"
                    >
                      <Card className="h-full overflow-hidden border-2 border-transparent hover:border-blue-300 hover:shadow-lg transition-all duration-300">
                        <div className="p-6">
                          <div className="flex items-start justify-between mb-3">
                            <h3 className="font-serif text-lg font-semibold text-foreground group-hover:text-blue-600 transition-colors line-clamp-2">
                              {facility.name}
                            </h3>
                          ...
                          <div className="flex items-center justify-between pt-3 border-t">
                            <span className="text-xs font-medium bg-blue-50 text-blue-700 px-2.5 py-1 rounded-full capitalize">
                              {facility.type?.replace(/-/g, ' ') || 'Loodgieter'}
                            </span>
              ...
              {results.length > visibleResults && (
                <div className="text-center py-8">
                  <p className="text-muted-foreground mb-4">
                    {visibleResults} van {results.length} resultaten
                  </p>
                  <Button
                    variant="outline"
                    size="lg"
          ...
          <section className="mt-16 pt-8 border-t border-blue-100">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {serviceTypes.slice(1).map((type) => (
                ...
                >
                  <Card className="p-5 h-full text-center hover:shadow-lg hover:border-blue-300 transition-all duration-300 group">
                    <div className="w-12 h-12 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center mx-auto mb-3 group-hover:bg-blue-600 group-hover:text-white transition-colors">
...
export default function SearchPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-background">
        <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 py-12">
