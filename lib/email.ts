...
const FROM_EMAIL = 'VindLoodgieter.nl <noreply@vindloodgieter.nl>';
...
export async function sendVerificationEmail(
  ...
  const subjects = {
    register: 'Verifieer uw e-mailadres - VindLoodgieter.nl',
    login: 'Uw inlogcode - VindLoodgieter.nl',
    claim: 'Verificatiecode voor uw claim - VindLoodgieter.nl',
  };

  const titles = {
    register: 'Welkom bij VindLoodgieter.nl',
    login: 'Uw inlogcode',
    claim: 'Verifieer uw claim',
  };

  const descriptions = {
    register: 'Bedankt voor uw registratie. Gebruik onderstaande code om uw e-mailadres te verifiëren.',
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: ${colors.foreground}; max-width: 600px; margin: 0 auto; padding: 20px; background-color: ${colors.background};">
  <div style="background-color: ${colors.white}; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(37, 99, 235, 0.08);">
    <div style="background: linear-gradient(135deg, ${colors.primary} 0%, ${colors.primaryLight} 100%); padding: 32px; text-align: center;">
      <h1 style="color: white; margin: 0; font-size: 24px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
        <span style="color: ${colors.white};">Vind</span><span style="color: ${colors.accent};">Loodgieter</span>
      </h1>
    </div>
    ...
    <div style="text-align: center; padding: 24px; color: ${colors.muted}; font-size: 12px; background-color: ${colors.background}; border-top: 1px solid ${colors.border};">
      <p style="margin: 0;">&copy; ${new Date().getFullYear()} VindLoodgieter.nl</p>
      <p style="margin: 5px 0 0 0;">
        <a href="https://www.vindloodgieter.nl" style="color: ${colors.accent};">vindloodgieter.nl</a>
      </p>
    </div>
  </div>
</body>
...
export async function sendWelcomeEmail(
  ...
  try {
    ...
    await resend.emails.send({
      from: FROM_EMAIL,
      to: [to],
      subject: 'Welkom bij VindLoodgieter.nl!',
      html: `
<!DOCTYPE html>
<html>
<head>
  ...
  <div style="background-color: ${colors.white}; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(37, 99, 235, 0.08);">
    ...
    <div style="padding: 32px;">
      ...
      <p style="color: ${colors.muted}; font-size: 16px;">
        Bedankt voor het aanmaken van een account bij VindLoodgieter.nl.
        Fijn dat u deel uitmaakt van ons platform!
      </p>
      ...
      <div style="background: ${colors.background}; border-radius: 8px; padding: 20px; margin: 24px 0; border: 1px solid ${colors.border};">
        ...
        <ul style="color: ${colors.muted}; padding-left: 20px; margin: 0;">
          ...
          <li style="margin-bottom: 8px;">Aanvragen van klanten ontvangen</li>
        </ul>
      </div>

      <div style="text-align: center; margin: 32px 0;">
        <a href="https://www.vindloodgieter.nl/dashboard"
      ...
      <p style="color: ${colors.muted}; font-size: 14px;">
        Beheert u een loodgietersbedrijf? Zoek uw bedrijf op en klik op "Claim deze vermelding"
      ...
      <p style="color: ${colors.muted}; font-size: 14px;">
        ...
        <a href="https://www.vindloodgieter.nl/contact" style="color: ${colors.accent};">ons contactformulier</a>.
      </p>
    </div>
...
export async function sendClaimApprovedEmail(
  ...
  try {
    ...
    await resend.emails.send({
      from: FROM_EMAIL,
      to: [to],
      subject: `Uw claim is goedgekeurd - ${loodgieterName}`,
      html: `
<!DOCTYPE html>
<html>
<head>
  ...
  <div style="background-color: ${colors.white}; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(37, 99, 235, 0.08);">
    ...
    <div style="padding: 32px;">
      ...
      <p style="color: ${colors.muted};">
        Goed nieuws! Uw claim voor <strong style="color: ${colors.foreground};">${loodgieterName}</strong> is goedgekeurd.
      </p>

      <p style="color: ${colors.muted};">
        U kunt nu inloggen op uw dashboard om de gegevens van uw bedrijf te beheren.
      </p>
